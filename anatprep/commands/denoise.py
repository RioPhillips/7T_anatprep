...
# Temporary filenames
    ...
    tmp_wsd = out.parent / f"tmp_wsd_{out.name}"
    ...
    # STEP 2: SANLM & BIAS CORRECTION (via sanlm_batch.sh)
    if run_sanlm or run_bias:
        logger.info("Step 2: Running SANLM/SPM BIAS correction")
        ...
        # If both SANLM and bias are skipped, the Heij output IS our SANLM/bias output
        # WSD step will still run on it
        tmp_heij.rename(out)
...
# STEP 3: WSD: Winsorize --> Rescale --> Datatype 
    ...
    tmp_winsorize = out.parent / f"tmp_winsorize_{out.name}"
    tmp_rescale = out.parent / f"tmp_rescale_{out.name}"

    # 3a: Winsorize intensities 
    #     ImageMath <dim> <out> TruncateImageIntensity <in> <lower> <upper> <bins>
    run_command([
        "ImageMath", "3", str(tmp_winsorize), "TruncateImageIntensity",
        str(out), "0.01", "0.99", "256",
    ], logger)

    # 3b: Rescale to [0, 4095] 
    #     ImageMath <dim> <out> RescaleImage <in> <min> <max>
    run_command([
        "ImageMath", "3", str(tmp_rescale), "RescaleImage",
        str(tmp_winsorize), "0", "4095",
    ], logger)

    # 3c: Set dtype to uint16 AND restore original header/affine
    img = nib.load(str(tmp_rescale))
    final_img = nib.Nifti1Image(img.get_fdata(), out_affine, out_header)
